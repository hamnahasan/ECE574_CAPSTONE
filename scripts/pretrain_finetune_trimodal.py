"""Two-phase training: pretrain TriModal Fusion U-Net (S1+S2+DEM) on the
WeaklyLabeled split, then fine-tune on the HandLabeled split.

Mirrors `pretrain_finetune_fusion.py` but with three modalities. Requires
that DEM tiles are aligned to the WeaklyLabeled chip grid (DEMWeak/) — that
alignment is a separate preprocessing job. If DEMWeak is missing, the
script will raise FileNotFoundError on the first weak batch.

Why this script:
  - The fusion-only weak pretrain (S1+S2) regressed test IoU vs scratch
    fine-tuning (0.7150 < 0.7686). The hypothesis was that the weak labels
    (Otsu thresholds) are too noisy for a 2-modal model to extract signal
    from. With DEM as a third anchor and the cross-attention bridge
    operating at three-way, the TriModal model may extract net positive
    signal from weak supervision where Fusion U-Net could not.
  - The result also informs the 0.81 U-Net ceiling finding: if TriModal +
    weak supervision (~4 385 chips) lifts test IoU into the 0.80+ range,
    the in-distribution ceiling is data-limited rather than architectural.

Outputs (mirror pretrain_finetune_fusion.py):
  results/checkpoints/{run_name}_pretrain_best.pt
  results/checkpoints/{run_name}_finetune_best.pt
...
Submission is via slurm/pretrain_finetune_trimodal.sbatch.
"""
...
def parse_args():
    ...
    p.add_argument("--run_name",         default="trimodal_weakpretrain")
...
# --------------------------------------------------------------------------
# Train / val loops (three-input variant)
# --------------------------------------------------------------------------
...
def train_one_epoch(model, loader, criterion, optimizer, scheduler, device, scaler, use_amp):
    ...
    for s1, s2, dem, labels in pbar:
        s1, s2, dem, labels = s1.to(device), s2.to(device), dem.to(device), labels.to(device)
        optimizer.zero_grad()
